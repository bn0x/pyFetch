...
def draw(options, args):
    ...
    line(ascii)
    line(ascii, "%sOS:      %s%s %s" % (Fore.RED, Fore.WHITE, system.os_release(), system.arch()['arch']))
    ...
    line(ascii, "%sRes:     %s%s%sx%s%s" % (Fore.RED, Fore.GREEN, res['x'], Fore.RED, Fore.GREEN, res['y']))
